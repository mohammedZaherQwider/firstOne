                    ...
                    <tbody class="fs-6">
                        @foreach ($operations as $operation)
                            <tr>
                                <td>
                                    ...
                                    <div class="d-flex align-items-center">
                                        ...
                                        <div class="d-flex flex-column justify-content-center">
                                            <a href=""
                                                class="fs-6 text-gray-800 text-hover-primary">{{ $operation->name }} </a>
                                            <div class="fw-bold text-gray-400">{{ $operation->id_patient }}</div>
                                        </div>
                                        <!--end::Info-->
                                    </div>
                                    <!--end::User-->
                                ...
                                <td>{{ $operation->hospital->name }}</td>
                                <td>{{ $operation->doctor->name }}</td>
                                <td>{{ $operation->specialization->name }}</td>
                                <td>{{ $operation->operation_name }}</td>
                                <td>{{ $operation->operation_date }}</td>
                                <td>{{ $operation->operation_time }}</td>
                                <td>{{ $operation->operation_schedule }}</td>
                                <td>
                                    @php
                                        $colors = [
                                    ...
                                    <span
                                        class="badge {{ $colors[$operation->status] ?? 'badge-light-secondary' }} fw-bolder px-4 py-3">
                                        {{ ucfirst($operation->status) }}
                                    </span>
                                </td>
                                <td>{{ $operation->price }} {{ $operation->pay ? '✅' : '❌' }}</td>
                                <td class="text-end">
                                    <div class="d-flex justify-content-end flex-shrink-0">
                                        ...
                                        @can('Delete Operation')
                                            <form class="d-inline" action="{{ route('operations.destroy', $operation->id) }}"
                                                method="POST">
                                                @csrf
                                                @method('delete')
